...
function [simuVel] = compare_signals_x(dataRoot,simuRoot,stationName,stationNumber,doPlot,hf)
...
%         xlim([0 20]);
        if (i == 1)
            title(['Station ' num2str(stationNumber) ': ' stationName ' --- Velocity (cm/s) max NS,EW: ' num2str(velMax(2,1)) ', ' num2str(velMax(2,2)) ' cm/s']);
        end
        ylabel(ylabels(i,:));
        if (i == 3)
            xlabel('Time (s)');
